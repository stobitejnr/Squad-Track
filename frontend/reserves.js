async function fetchData() {
    try {
        const response = await fetch('http://98.83.29.224:3000/reserves'); // Fetch data from the server
...
async function deleteRow(rowId) {
    ...
    // Send DELETE request
    let response = await fetch("http://98.83.29.224:3000/reserver", {
        method: "DELETE",
        body: bodyContent,
        headers: headersList
    });
...
function saveData() {
    ...
    async function postData() {
        ...
        let response = await fetch("http://98.83.29.224:3000/reserves", {
            method: "POST",
            body: bodyContent,
            headers: headersList
        });
