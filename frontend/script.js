async function fetchData() {
    try {
        const response = await fetch('http://54.145.179.13:3000/first'); // Fetch data from the server
...
async function deleteRow(rowId) {
    ...
    // Send DELETE request
    let response = await fetch("http://54.145.179.13:3000/firster", {
        method: "DELETE",
        body: bodyContent,
        headers: headersList
    });
...
function saveData() {
    ...
    async function postData() {
        ...
        let response = await fetch("http://54.145.179.13:3000/first", {
            method: "POST",
            body: bodyContent,
            headers: headersList
        });
